...
Create, Recover, Recent, Advanced, and Settings paths. A fresh guided Create
workflow selects High Capacity by default; the CLI, public API, and classic
low-level paths retain Resilient defaults for compatibility. English and
Turkish can be switched at runtime from the header or Settings without
clearing the current workflow. The chosen language is saved for the next
launch.
...
|---|---:|---:|---:|---|---|
| High Capacity | 4x4 | 1 | 5% | Higher useful capacity / shorter videos; fresh guided Create default | Real YouTube roundtrip, 6/6 exact |
| Balanced | 8x8 | 1 | 20% | General-purpose experiments | Repair-overhead preset |
| Resilient | 8x8 | 1 | 5% | Conservative geometry; CLI/API compatibility default | Existing production baseline |
...
High Capacity is the default for a fresh guided GUI Create workflow. The CLI,
public C API, and established classic paths continue to default to Resilient
for compatibility. Resilient remains the more conservative geometry.
...
Reliability profiles themselves do not split inputs. The Video Set layer
described below provides multi-video splitting without changing these
...
```powershell
& "C:\Users\<USERNAME>\Reliquary\build\vcpkg_installed\x64-windows\tools\Qt6\bin\windeployqt.exe" --release "C:\Users\<USERNAME>\Reliquary\build\Release\media_storage_gui.exe"
```
...
controls. The classic Storage path retains Resilient as its compatibility
default; a fresh guided Video Set Create path selects High Capacity. High
Capacity shows its 4x4, one-bit, signal 1.0, 5% repair parameters and the
six-case real YouTube validation note; changing repair manually continues to
require Custom.
...
The current Windows test build discovers and passes **500/500 tests**:
...
signal-1.0/repair-5% configuration is available as the High Capacity profile.
Resilient remains the more conservative alternative, and
neither profile is an absolute guarantee.
...
Video Set is the file-only archive layer for sources that should be
...
High Capacity is the preselected **Fewer and Shorter Videos** mode for a fresh
Create workflow. Resilient remains the user-selectable **Most Reliable**
alternative and the CLI/API compatibility default. The interface keeps the
separate 6/6 single-video and 4/4 Video Set validation evidence visible for
High Capacity. Duration, actual-size cap, reserve, geometry, config ID, part
ranges, and technical logs remain available under **Advanced settings**,
**Show part details**, and **Advanced / Classic Video Set Tools**, but are
...
The GUI exposes the same workflow in the **Video Sets** tab: source,
...
The fresh guided Create path defaults to High Capacity (4x4, one bit, signal
1.0, 5% repair, config `538F2B009FAB`), which was exact in 6/6 real YouTube
single-video stress cases. Resilient (8x8, one bit, signal 1.0, 5% repair)
remains the conservative alternative and CLI/API default. No profile is an
absolute data guarantee. Video Set version 1 intentionally omits cross-video
parity, multi-source/folder archives, compression redesign, and multi-video
stream encode. Optional OAuth upload automation remains isolated under the
experimental YouTube Sync feature.

## Compatibility

The user-facing product name is Reliquary, but compatibility identifiers are
deliberately stable. Existing single-video formats remain decodable, and
legacy VidStoreX-era Video Sets using the embedded `VSXSET01` envelope remain
readable. The current executable names are still `media_storage.exe` and
`media_storage_gui.exe`; no packaged `Reliquary.exe` is implied.

## YouTube disclaimer

YouTube is not a guaranteed archival storage service. Uploads may be
re-encoded, and platform behavior, policies, or limits can change. The 6/6
single-video and 4/4 Video Set results describe observed real roundtrips, not
a promise about future uploads. Reliquary considers recovery successful only
when the recovered file's complete SHA-256 equals the original value.

Keep an independent trusted backup of important data.

## Author

Reliquary is developed by **Burhan Talha Yazıcı (BTY)**.
